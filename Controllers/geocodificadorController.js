...
const pool = require('../../config/db'); // Se importa la configuración de la base de datos
const { sendErrorResponse, sendSuccessResponse } = require("../../utils/responseHandler"); // Se importan las funciones para manejar las respuestas

// FUNCIÓN PARA REALIZAR BÚSQUEDAS DE INFORMACIÓN
const buscador_info = async (req, res) => {
    try {
        ...
        if (id_region != undefined || id_region != null) {
            ...
            let respuesta = await pool.query(`SELECT points, intersects FROM  
                ...
                FROM carto_region r WHERE r.id_region = ANY ($3::INT[])) as t1 WHERE intersects = true`, [input.long, input.lat, idRegion]);
                ...
                let estado = await pool.query(queryEstado, [respuesta.rows[0].points, params.region.id_region]);
                ...
                let municipio = await pool.query(queryMunicipio, [respuesta.rows[0].points, params.region.id_region, params.estado.id_estado]);
                ...
                let localidad = await pool.query(queryLocalidad, [respuesta.rows[0].points, params.region.id_region, params.estado.id_estado, params.municipio.id_municipio]);
                ...
                let ageb = await pool.query(queryAgeb, [respuesta.rows[0].points, params.region.id_region, params.estado.id_estado, params.municipio.id_municipio]);
                ...
                filtros.push(respuesta.rows[0].points, params.region.id_region, params.estado.id_estado, params.municipio.id_municipio);
                ...
                let calle = await pool.query(queryCalle, [respuesta.rows[0].points, params.region.id_region, params.municipio.id_municipio, params.estado.id_estado]);
                if (calle.rowCount > 0) {
                ...
                let poi = await pool.query(queryPoi, [respuesta.rows[0].points, params.region.id_region]);
